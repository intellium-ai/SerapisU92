...
def train_predictor(data_path: str, pretrained_path: str, target_index=1, epochs=100, k=10, SMILE_enumeration_level=100):
    """
    :param data_path:
    ...
    for i in range(k):
        ...
        losses = reg_rnn.train(model=model, optimizer=opt, train_loader=loader, valid_x=test_oh, valid_y=test_y,
                               epochs=epochs, name=name_of_m)

        tools.save_data_to_csv(f'record/REG_{SMILE_enumeration_level}_FOLD-{i}_LOSS.csv', losses, ['epoch', 'train_loss', 'valid_loss'])
        ...
        tools.save_data_to_csv(f'record/REG_{SMILE_enumeration_level}_FOLD-{i}_RESULTS.csv', results, ['smi', 'y', 'y_pred', 'test'])
    ...
    tools.save_data_to_csv(f'record/REG_{SMILE_enumeration_level}_Performance.csv', rcd,
